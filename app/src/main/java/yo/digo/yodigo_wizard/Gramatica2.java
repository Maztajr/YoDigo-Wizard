...
public class Gramatica2 extends Gramatica {
	
	ArrayList<Integer> added;

	public Gramatica2(ArrayList<Elemento> m, Context con){
		super(m,con);
			...
			if(ban==0){
				if(e.getClasificacion() == DatabaseHelper.PRONOMBRE_PERSONAL){
					...
					checkAddedElements(1);
						...
						e.getClasificacion() == DatabaseHelper.PREPOSICION ||
						e.getClasificacion() == DatabaseHelper.ARTICULO){
					mensaje.set(2, e);
